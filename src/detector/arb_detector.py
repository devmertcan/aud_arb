            ...
            for j in range(i + 1, len(ex_names)):
                ...
                if bid_a and ask_b:
                    ...
                    if spread > ask_b[0] * self.threshold:
                        self.report(f"[OPPORTUNITY] Buy {ex_b} @ {ask_b[0]} / Sell {ex_a} @ {bid_a[0]} | Spread {spread:.2f}")

                if bid_b and ask_a:
                    spread = bid_b[0] - ask_a[0]
                    if spread > ask_a[0] * self.threshold:
                        self.report(f"[OPPORTUNITY] Buy {ex_a} @ {ask_a[0]} / Sell {ex_b} @ {bid_b[0]} | Spread {spread:.2f}")
